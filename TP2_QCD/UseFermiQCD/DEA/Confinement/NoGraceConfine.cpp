...
/// /////////////////////
  int ndim=4;
  int tsize=20,xsize=20,ysize=6,zsize=6;                       ///lattice size   tsize=xsize is compulsory
  ...
  site x(U.lattice());
  int systemchoice=2;                        /// systemchoice=0   is  faster but implemented only  with nc=2
                                             ///systemchoice=2 use the slow c++ function but works for any nc
/// /////////////////////
/// ITERATION CONTROL //
/// ///////////////////
  int decorel=5,nheat=700;                    /// decorrelation and heating
  int maxiter=3000,niter=2100;                /// iterations
...
/// /////////////////////////////////////////////////////////////////////////////////////////////////
//
  int maxloopsize=5;                          /// maximum loop size in t,x directions, not larger than 10
...
// 1. On multiplie par 4 (pour les 4 directions mu) pour être sûr de couvrir l'indexation de full_adress
 // int fullsize = box[0] * box[1] * box[2] * box[3] * nc * nc *4 ; 
 // float * RfullCCfield_t,* IfullCCfield_t,* RfullCCfield_x,* IfullCCfield_x;

// 2. Utilise calloc au lieu de malloc (cela initialise à zéro et aide à détecter les erreurs)
 // RfullCCfield_t = (float*)calloc(fullsize, sizeof(float));
 // IfullCCfield_t = (float*)calloc(fullsize, sizeof(float));
 // RfullCCfield_x = (float*)calloc(fullsize, sizeof(float));
 // IfullCCfield_x = (float*)calloc(fullsize, sizeof(float));
...
//
  ...
  float* store_loop = (float*)malloc((maxiter+1) * zmax * tmax * sizeof(float));
  if(store_loop == NULL) { cout << "Erreur allocation store_loop"; abort(); }
...
//
   if(systemchoice==0)
    {t_loop=(float)clock();
//  make_aver_loops_with_CPU(RfullCCfield_t,IfullCCfield_t,RfullCCfield_x,IfullCCfield_x,box,nc,CPU_average_loop);
//
      ...
      for(int tloop=0;tloop<tmax;tloop++)
       ...
       //  store_loop[k * (zmax * tmax) + zloop * tmax + tloop] =CPU_average_loop[iloop];
         }
...
//
   ...
   if(systemchoice==2)                                  ///slow option, normally used only for checks
    {t_loop=(float)clock();
      for(int zloop=0;zloop<zmax;zloop++)              //computes and store the loops
      for(int tloop=0;tloop<tmax;tloop++)
	     {store_loop[k * (zmax * tmax) + zloop * tmax + tloop] =my_average_loop(U,mu,tvalue[tloop],nu,zvalue[zloop]);}
  ...
  {ofstream tmp("tmp.dat");                  //stream for pollting v(t,z)
	...
	for(int tloop=0;tloop<tmax;tloop++)
	      ...
	      for(int kp=1;kp<=k;kp++)                //fill jack with data from 1 to current k
	           {jack(kp-1,1)=store_loop[kp * (zmax * tmax) + zloop * tmax + tloop];
	           jack(kp-1,0)=1.;
	           if(tloop>0){jack(kp-1,0)=store_loop[kp * (zmax * tmax) + zloop * tmax + tloop-1]; }
	           }
